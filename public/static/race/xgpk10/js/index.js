...
let horseIssueDetail = document.querySelector('.horseIssueDetail');
...
window.addEventListener('message', function (msg) {
    if (msg.data.lottery === 'xgpk10') {
        if (msg.data.type === 'mounted') { //首次進入頁面執行這個渲染底部排序
            let openDataStr = msg.data.data.code.replace(/([0-9])(?=(\d{2})+$)/g, '$1,');
