...
import { lotteryAjax, traceAjax, agyxAjax, fishAjax, ptyxAjax, bbinAjax, sportAjax, idnAjax, kgameAjax, statisticsAjax } from './ajax'
...
class Game extends React.Component {
    ...
    onChange(e) {
        ...
        //切换TAB重置时间初始值
        const starValue = this.state.starValue = timeDay('start');
        const endValue = this.state.endValue = timeDay('end');
    ...
    handsumit(radioState, size) {
        ...
        if (config[radioState].Ajax) {
            ...
            config[radioState].Ajax(ds).then(res => {
                if (res.data && res.data.code == '1') {
                    const d = this.state.d = res.data.result
    ...
    render() {
        ...
        const { starValue, endValue, defaultRadios, d } = this.state
